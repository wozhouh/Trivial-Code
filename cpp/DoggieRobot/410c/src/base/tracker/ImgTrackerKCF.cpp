...
void ImgTrackerKCF::init(const cv::Rect &roi, cv::Mat image)
{
    ...
    train_init(_tmpl); // train with initial frame
 }
...
void ImgTrackerKCF::train_init(cv::Mat x)
{
    using namespace FFTTools;
...
void ImgTrackerKCF::train(cv::Mat x, float train_interp_factor)
{
    ...
    _tmpl = (1 - train_interp_factor) * (1-init_R) * _tmpl + (train_interp_factor) * (1-init_R) * x + init_R * tmpl_init;
    _alphaf = (1 - train_interp_factor) * (1-init_R) * _alphaf + (train_interp_factor) * (1-init_R) * alphaf + init_R * alpha_init;
}
